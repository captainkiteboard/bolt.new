...
import { JobMatchResponse, performJobMatch } from '../middleware/jobmatch';
...
const HomePage = () => {
  ...
  const handleFileUpload = useCallback((file: File, type: 'resume' | 'jobDescription') => {
  ...
  const handleMatch = async () => {
    ...
    try {
      const result: JobMatchResponse = await performJobMatch(resume, jobDescription);
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <UploadSection
          ...
          onChange={(value: string) => {
            const file = new File([value], "resume");
        ...
        <UploadSection
          ...
          value={jobDescription ? jobDescription.name : ''}
          onChange={(value: string) => {
            const file = new File([value], "jobDescription");
            handleFileUpload(file, 'jobDescription');
          }}
